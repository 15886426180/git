...
void CandidateArmor::inputParam(NiceLight &light_left, NiceLight &light_right, RM_SolveAngle &angle_solve, ArmorROI &roi)
{
    ...
    convert_center = rect.center + Point2f(roi.tl);
...
void RM_ArmorFitted::armorFitted()
{
    ...
    findContours(bin_img_color, contours, RETR_EXTERNAL, CHAIN_APPROX_NONE);
    int perimeter = 0;
    for (size_t j = 0; j < contours.size(); ++j)
    {
        perimeter = arcLength(contours[j], true); //轮廓周长
        if (contours[j].size() < 6 && perimeter > 4000 || perimeter < 50)
        {
            continue;
        }
        ...
        if ((w_h_ratio < 0.4f && R_rect.angle > 45 && R_rect.angle < 135) /*高宽比,角度筛选形状符合要求的灯条*/
            && ((0 <= R_rect.angle && R_rect.angle <= 45) || (135 <= R_rect.angle && R_rect.angle <= 180)))
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                line(dst_img, vtx[j] + (Point2f)roi.tl, vtx[(j + 1) % 4] + (Point2f)roi.tl, Scalar(200, 0, 0), 2, 8, 0);
            }
    ...
    for (size_t i = 0; i < candidate_lights.size(); ++i)
    {
        for (size_t j = i + 1; j < candidate_lights.size(); ++j)
        {
            ...
            if (is_height_diff_catch && is_light_angle_catch && is_light_distance_catch)
            {
...
//发送串口数据
#if IS_SERIAL_OPEN == 1
    SerialPort::RMserialWrite(_yaw_data, abs(yaw_data), _pitch_data, abs(pitch_data), armor.depth, is_last_data_catch, shooting);
#endif
